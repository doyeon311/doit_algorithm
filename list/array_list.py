...
class ArrayLinkedList:
    ...
    def get_insert_index(self):
        """다음에 삽입할 레소크의 인덱스를 구함"""
        if self.deleted == Null:                # 삭제된 레코드는 존재하지 않음
    ...
    def delete_index(self, idx: int) -> None:
        ...
        if self.deleted == Null:                # 삭제 레코드는 존재하지 않음
            ...
            rec = self.deleted
            self.deleted = idx                  # idx를 피리 리스트의 맨 앞에 삽입
    ...
    def remove(self, p: int) -> None:
        ...
        if self.head != Null:
            if p == self.head:
                self.remove_first()                 # p가 머리 노드면 머리 노드를 제거
            else:
                ptr = self.head
