...
class FixedStack:
    ...
    def __len__(self) -> int:
        ...
        # __len__함수를 정의하면 a.__len__()의 형태로 호출할 수 있고 간단히 len(a)로도 호출할 수 있다.
        return self.ptr
    ...
    def clear(self) -> None:
        """스택을 비움(모든 데이터를 삭제)"""
        self.ptr = 0 # 스택에서 push나 pop등 모든 작업은 스택포인터를 바탕으로 이루어진다. 따라서 스택의 배열 원솟값을 변경할 필요가 없다.
    ...
    def __contains__(self, value: Any) -> bool:
        """스택에 value가 있는지 판단"""
        # `a.__contains__(2)`의 형태로도 호출할 수 있지만 간단히 `2 in a`의 형태로도 호출할 수 있다.
